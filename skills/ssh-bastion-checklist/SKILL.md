...
description: Use when `mysql_exec` (or any tools-mcp call with `tunnel=ssh`) fails at the SSH layer — connect failure, auth failure, direct-tcpip channel failure, or a multi-hop chain that drops between hops. Walks through diagnostic checks before changing the call.
---

# SSH bastion / tunnel troubleshooting checklist

`tools-mcp`'s SSH tunnel walks the chain `client → bastion1 → … → bastionN → target_host`. A failure in any of those edges produces a different error message. Pick the matching section, run the listed checks, then propose a fix.
...
4. **Both supplied**: tools-mcp tries publickey first, then falls back to password. If publickey fails for any reason (bad key file, permissions, key not in `authorized_keys`), the password attempt happens next — so a `password authentication rejected` error means the FALLBACK also failed.
...
To verify the tools-mcp tunnel itself works (separating "tunnel broken" from "MySQL broken"):

```bash
tools-mcp --tunnel=ssh --ssh-jump=<bastion> --ssh-user=<user> --ssh-password=<pwd> \
