//! `pgsql_exec` MCP tool — params + `McpTool` impl.

use crate::orchestrator::{PgsqlOrchestrator, PgsqlRequest};
...
#[async_trait]
impl McpTool for PgsqlMcp {
    const NAME: &'static str = "pgsql_exec";
    const DESCRIPTION: &'static str = "Execute a PostgreSQL query, optionally through an SSH jump host. \
         Reads are allowed by default; writes (INSERT/UPDATE/DELETE/DDL) require allow_write=true.";
    type Params = PgsqlExecParams;
...
fn params_to_config(p: &PgsqlExecParams, toml: Option<TomlConfig>) -> Result<Config> {
    ...
    if let Some(profile_name) = &p.profile {
        ...
        let (_, profile_cfg) =
            toml_config.resolve_profile(profile_name, Some(ServiceType::Pgsql))?;
