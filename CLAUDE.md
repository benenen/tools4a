...
`tools-mcp` is a Rust CLI + MCP server for HTTP, MySQL, Redis, and SSH. **Phase 8 (current) introduces a unified `Service` trait in `tools-mcp-core` whose `async fn execute(Self::Request, Option<TunnelConfig>) -> Result<ExecutionResult>` shape is implemented by `MysqlOrchestrator`, `RedisOrchestrator`, `HttpOrchestrator`, and `SshDirectOrchestrator` — all four now living in the new `tools-mcp-orchestrator` crate alongside the `Config` / `Profile` / `Loader` / `Merger` types and the `DirectTunnel` / `SshTunnel` runtime impls. The bin shrinks to `cli/`, `mcp/`, `output/`, `connection/`, `main.rs`.** All four supported services still ship as both CLI subcommands (`mysql`, `redis`, `http`, `ssh`) and MCP tools (`mysql_exec`, `redis_exec`, `http_exec`, `ssh_exec`). The design spec lives at `docs/superpowers/specs/2026-05-07-tools-mcp-design.md`; recent plans are in `docs/superpowers/plans/`.
...
**Workspace.** The repo is a Cargo workspace. The `tools-mcp` binary crate lives at the repo root (presentation layer only: `cli/`, `mcp/`, `output/`, `connection/`, `main.rs`, `lib.rs`). The lib crates under `crates/` are: `tools-mcp-core` (trait floor + `TunnelConfig` + `Service` trait), `tools-mcp-mysql`, `tools-mcp-redis`, `tools-mcp-http`, `tools-mcp-ssh` (each owns one service's primitives), and `tools-mcp-orchestrator` (NEW in Phase 8 — owns the `Config` / `Profile` / `Loader` / `Merger` types, `DirectTunnel` / `SshTunnel` runtime impls, and the four `<Svc>Orchestrator: impl Service` types). Adding a new service typically means a new `tools-mcp-<svc>` lib crate plus a `<Svc>Orchestrator` in `tools-mcp-orchestrator`.
...
| `tools-mcp-orchestrator` (lib, NEW Phase 8) | `Config` / `Profile` / `ConfigLoader` / `ConfigMerger` (3-layer merge primitives); `DirectTunnel` / `SshTunnel` runtime impls (impl `core::Tunnel`); `MysqlOrchestrator` + `MysqlRequest` (with `from_config` ctor); `RedisOrchestrator` + `RedisRequest` (with `from_config`); `HttpOrchestrator` (uses `tools_mcp_http::HttpRequestSpec` directly — no `from_config` since HTTP defers Profile/YAML); `SshDirectOrchestrator` (uses `tools_mcp_ssh::SshExecRequest` directly — same reasoning). Each `<Svc>Orchestrator` impls `core::Service`. Deps: `async-trait` + `serde` + `serde_yml` + `toml` + all 5 service libs + `russh` + `reqwest` (URL parsing only) + `tokio`. |
| `tools-mcp` bin (`src/cli/*`) | clap `Cli`, `SshTunnelArgs`, `CliHandler` — CLI mode parse + dispatch. Builds typed `<Svc>Request` (via `<Svc>Request::from_config` for mysql/redis, directly for http/ssh) and calls `<Svc>Orchestrator::execute(req, tunnel)`. |
| `tools-mcp` bin (`src/mcp/*`) | rmcp `ServerHandler`, four `<svc>_exec` tools — same flow as CLI handler, just with JSON params instead of clap structs. |
